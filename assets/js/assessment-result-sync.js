...
function resultBelongsToAttempt(attempt, result) {
...
function lppResult() {
  let state;
  try {
    state = JSON.parse(localStorage.getItem('tamayuz10x-lpp-v1') || 'null');
  } catch (_) {
    return null;
  }
  ...
  const sorted = order
    .map(code => ({ code, score: percent[code] }))
    .sort((a, b) => b.score - a.score || order.indexOf(a.code) - order.indexOf(b.code));
  return {
...
async function ensurePdfReport(attemptId, assessmentKey, result) {
  if (reportChecked.has(attemptId)) return;
  const existing = await listAssessmentReports().catch(() => ({ reports: [] }));
  const ready = (existing?.reports || []).filter(r => r.attempt_id === attemptId && r.status === 'ready');
  if (ready.some(r => String(r.file_name || '').includes('-visual-v3-'))) {
    reportChecked.add(attemptId);
    return;
  }
  const pdf = await createAssessmentReportPdf(assessmentKey, result);
  const suffix = result?.language === 'en' ? 'en' : 'ar';
  await uploadAssessmentReport(attemptId, pdf, `${assessmentKey}-visual-v3-${suffix}.pdf`);
  reportChecked.add(attemptId);
}
...
async function sync(assessmentKey, result) {
  ...
  if (synced.has(attempt.id)) return;
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent('tamayuz:assessment-result-saved', {
