...
const root = document.getElementById('trainerSessions');
if (root) {
  ...
  const t = lang === 'ar' ? {
    ...
    completed: 'أكملوا التقييم',
    code: 'رمز الجلسة',
    open: 'مفتوحة',
    archived: 'مؤرشفة',
    results: 'عرض النتائج',
    share: 'فتح رابط المشاركين',
  ...
  async function load() {
    ...
    try {
      ...
      root.innerHTML = sessions.map((s) => {
        ...
        return `<article class="attempt trainer-session-row" data-trainer-session="${esc(s.session_code)}">
          <div class="row">
            <div>
              <strong>${esc(s.session_name || (lang === 'ar' ? 'جلسة تفضيلات التعلّم' : 'Learning preference session'))}</strong><br>
              <span class="muted">${t.code}: <b dir="ltr">${esc(s.session_code)}</b> · ${status} · ${esc(dateText(s.created_at))}</span>
            </div>
            <div class="metric" style="min-width:120px;text-align:center"><strong>${esc(n)}</strong><span>${t.completed}</span></div>
          </div>
          <div class="attempt-actions">
            <a class="btn" href="${dashboard}">${t.results}</a>
            ${s.join_url ? `<a class="btn secondary" href="${esc(s.join_url)}" target="_blank" rel="noopener">${t.share}</a>` : ''}
            <button class="btn secondary" data-session-status="${s.status === 'archived' ? 'open' : 'archived'}" data-session-code="${esc(s.session_code)}">${s.status === 'archived' ? t.restore : t.archive}</button>
          </div>
        </article>`;
      }).join('');
      root.querySelectorAll('[data-session-status]').forEach((btn) => {
        btn.addEventListener('click', async () => {
          ...
          btn.disabled = true;
          try {
            await setTrainerSessionStatus(btn.dataset.sessionCode, btn.dataset.sessionStatus);
            await load();
          } catch (e) {
            console.error(e);
            btn.disabled = false;
      ...
      console.error('Trainer sessions load failed:', e);
      root.textContent = t.failed;
    }
  }
