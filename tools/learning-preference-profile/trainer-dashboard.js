(() => {
  ...
  async function load(){
    ...
    try{
      ...
      render(d);
    }catch(e){renderError(tx('The group dashboard could not be reached. Please refresh and try again.','تعذر الوصول إلى لوحة المجموعة. حدّث الصفحة وحاول مرة أخرى.'));}
  }
  function render(d){
    const avg=d.averages||{}; const dist=d.distribution||{}; const n=d.participant_count??d.completed_participants??0;
    ...
    root.innerHTML=`
      ...
      ${hasDistribution?`<div class="lpp-section-block"><h2>${tx('Leading-profile distribution','توزيع الملفات البارزة')}</h2><div class="lpp-meaning-grid">
      ...
      <div class="lpp-section-block"><h2>${tx('How to use this view','كيف تستخدم هذه اللوحة؟')}</h2><p class="lpp-profile-sub">${tx('Use the averages to balance the session across practice, reflection, conceptual understanding, and application. Do not label the whole group with one fixed type.','استخدم المتوسطات لموازنة الجلسة بين الممارسة والتأمل والفهم المفاهيمي والتطبيق. لا تصنّف المجموعة كلها في نمط ثابت واحد.')}</p></div>`;
